...
const { toHiragana, toKatakana, toRomaji } = romajiConv;
...
describe('基本的な五十音の確認', () => {
  let romaji, simpleRomaji, hiragana, katakana;
  ...
  beforeEach(() => {
    ...
    simpleRomaji = '' +
      'aiueokakikukekosashisusesotachitsutetonaninuneno' +
      'hahifuhehomamimumemoyayuyorarirurerowawiuwewonn';
  ...
  test('ひらがな→ローマ字', () => {
    expect(hiraganaInstance.toRomaji()).toBe(simpleRomaji);
  });

  test('ひらがな→ローマ字（関数版）', () => {
    expect(toRomaji(hiragana)).toBe(simpleRomaji);
  });
});
...
test('toRomaji: 拗音', () => {
  expect(toRomaji('しょう')).toBe('shou');
  expect(toRomaji('しょ')).toBe('sho');
  expect(toRomaji('ひょう')).toBe('hyou');
  expect(toRomaji('ひょ')).toBe('hyo');
  expect(toRomaji('きょう')).toBe('kyou');
  expect(toRomaji('きょ')).toBe('kyo');
  expect(toRomaji('きょうと')).toBe('kyouto');
  expect(toRomaji('ちょう')).toBe('chou');
...
test('toRomaji: 促音', () => {
  ...
  expect(toRomaji('しょっき')).toBe('shokki');
});

test('toRomaji: Convert 「ん」 to "nn"', () => {
  expect(toRomaji('なんで', true)).toBe('nannde');
  expect(toRomaji('にゅ', true)).toBe('nyu');
  expect(toRomaji('なんい', true)).toBe('nanni');
});
