...
describe('わ行の ゐ・ゑ の相互変換', () => {
  test('ひらがな→カタカナ', () => {
    expect(toKatakana('ゐ')).toBe('ヰ');
    expect(toKatakana('ゑ')).toBe('ヱ');
  });

  test('カタカナ→ひらがな', () => {
    expect(toHiragana('ヰ')).toBe('ゐ');
    expect(toHiragana('ヱ')).toBe('ゑ');
  });

  test('カタカナ→ひらがな（ヱビス）', () => {
    expect(toHiragana('ヱビス')).toBe('ゑびす');
  });
});
